...
if ([string]::IsNullOrWhiteSpace($ArchivePath)) { $ArchivePath = Join-Path $PSScriptRoot 'dist\MyVibe-0.3.1-beta.1-win-x64.zip' }
...
try {
    ...
    if ($version -ne '0.3.1.0') { throw "Unexpected MyVibe file version: $version" }
